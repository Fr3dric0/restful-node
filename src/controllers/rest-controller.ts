...
import { NotFoundError, MethodNotAllowed, BadRequestError } from '../errors/http.error';
...
export default class RestController {
    ...
    async list(req, res, next) {
        ...
        let document;
        try {
            document = await this.model.find(req.body);
        } catch (e) {
            return next(e);
        }
    ...
    async updateWrapper(req, res, next) {
        ...
        if (!data) {
            return next(new NotFoundError(`Cannot update unknown resource: ${req.params[this.pk]}`));
        }
